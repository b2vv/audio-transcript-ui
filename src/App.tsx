...
import {TranscriptionFile, TranscriptionService} from '@src/api/transcription.service.ts';
...
function App() {
    ...
    const [transcriptions, setTranscriptions] = useState<TranscriptionFile['segments']>([]);
    ...
    const submitHandler = React.useCallback((formData: FormData) => {
        ...
        api.sendAudio(formData).then(({data}) => {
            setTranscriptions((prev) => [...prev, ...data.segments]);
            if (listRef.current) {
